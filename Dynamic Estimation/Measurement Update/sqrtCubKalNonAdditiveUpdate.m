...
%                      measurement covariance matrix in the native
%                      coordinate system of the measurement.
%           h          A function handle for the measurement function. This
%                      can either be of the form h(xStacked), where
%                      xStacked is [x;w], where x is the state and w is the
%                      measurement noise value, or this can be of the form
%                      h(x,w), where the inputs are separately specified.
%           xi         An (xDim+zDim) X numCubPoints matrix of cubature
%                      points. If this and the next parameter are omitted
%                      or empty matrices are passed, then
%                      fifthOrderCubPoints(xDim) is used. It is suggested
...
%                      linear. For example, when averaging angular values,
%                      then the function meanAng should be used.
